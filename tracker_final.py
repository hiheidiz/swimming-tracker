...
def process_video_multitrack(video_path, interpolate_max_gap=0.25, smoothing_iterations=3):
    """
    ...
    Saves output video and CSV to a folder named like the video (without extension).
    """
    import os
    ...
    # Output folder: same name as video (without extension)
    video_basename = os.path.splitext(os.path.basename(video_path))[0]
    video_dir = os.path.dirname(os.path.abspath(video_path)) or os.getcwd()
    out_dir = os.path.join(video_dir, video_basename)
    os.makedirs(out_dir, exist_ok=True)
    out_dir = os.path.abspath(out_dir)

    # Write to a temp file first (no spaces, short path) then move to final path — avoids OpenCV writer failures on macOS
    out_path = os.path.join(out_dir, f"tracked_{video_basename}.mp4")
    fd, temp_path = tempfile.mkstemp(suffix=".mp4", prefix="tracked_", dir=tempfile.gettempdir())
    os.close(fd)
    try:
        os.remove(temp_path)  # mkstemp created empty file; remove so VideoWriter can create it
    except OSError:
        pass
    for codec_name, fourcc in [('avc1', cv2.VideoWriter_fourcc(*'avc1')),
                               ('mp4v', cv2.VideoWriter_fourcc(*'mp4v')),
                               ('H264', cv2.VideoWriter_fourcc(*'H264'))]:
        out_writer = cv2.VideoWriter(temp_path, fourcc, fps, (width, height))
        if out_writer.isOpened():
            print(f"[INFO] Writing tracked video (codec: {codec_name}), will save to {out_path}")
            break
        out_writer.release()
    else:
        raise IOError(f"Failed to open video writer. Try installing ffmpeg: brew install ffmpeg")
    ...
    while True:
        ...
        if x_ref is None:
            # No line detected: use center of the drawn box
            rx, ry, rw, rh = line_region
            x_ref = rx + rw / 2.0
        prev_line_x = x_ref
        line_x_list.append(x_ref)

        # Recenter the line region box on the detected line so it tracks with the line
        rx, ry, rw, rh = line_region
        new_rx = int(x_ref - rw / 2)
        new_rx = max(0, min(new_rx, width - rw))
        line_region = (new_rx, ry, rw, rh)
        ...
        for i, tracker in enumerate(trackers):
            ...
            if success:
                ...
                # Remove if within 5 pixels of any frame edge
                margin = 5
                if cx < margin or cx >= width - margin or cy < margin or cy >= height - margin:
                    print(f"[INFO] Point {obj_id} near frame edge at frame {frame_idx}, removing")
                    to_remove.append(i)
                    continue
    ...
    out_csv = os.path.join(out_dir, f"points_{video_basename}.csv")
...
Output: Video and CSV are saved to a folder with the same name as the input video.
        """
    )
    ...
    df = process_video_multitrack(args.video)
